...
# >>> ADD weekday (English, Europe/Tallinn) to HOURLY (minimal change) <<<
if not sum_hourly_el_consumption.empty:
    ser = sum_hourly_el_consumption["sum_cons_time"]
    if ser.dt.tz is None:
        ser = ser.dt.tz_localize("UTC")
    # UTC-based weekday
    sum_hourly_el_consumption["weekday"] = ser.dt.day_name()
    sum_hourly_el_consumption["is_weekend"] = sum_hourly_el_consumption["weekday"].isin([
                                                                                        "Saturday", "Sunday"])
# >>> ADD Estonia public holidays (local date in Europe/Tallinn; DF stays UTC) <<<
try:
    import holidays
    # Determine covered years from UTC timestamps
    dt_min = sum_hourly_el_consumption["sum_cons_time"].min()
    dt_max = sum_hourly_el_consumption["sum_cons_time"].max()
    if pd.notna(dt_min) and pd.notna(dt_max):
        ...
        local_dates = sum_hourly_el_consumption["sum_cons_time"].dt.tz_convert(
            "Europe/Tallinn").dt.date
        sum_hourly_el_consumption["is_holiday"] = local_dates.map(
            lambda d: d in ee_holidays)
    else:
        sum_hourly_el_consumption["is_holiday"] = False
except Exception as _e:
    print("[warn] holidays library missing or failed; set hourly is_holiday=False. Install: pip install holidays")
    sum_hourly_el_consumption["is_holiday"] = False
...
# >>> ADD weekday (English) to DAILY (minimal change; label based on local midnight) <<<
if not sum_daily_el_consumption.empty:
    sum_daily_el_consumption["weekday"] = pd.to_datetime(
        sum_daily_el_consumption["sum_cons_date"]
    ).dt.day_name()  # UTC-based weekday
    sum_daily_el_consumption["is_weekend"] = sum_daily_el_consumption["weekday"].isin([
                                                                                      "Saturday", "Sunday"])
# >>> ADD Estonia public holidays to DAILY (label by local EE calendar day name) <<<
try:
    import holidays
    if not sum_daily_el_consumption.empty:
        dmin = sum_daily_el_consumption["sum_cons_date"].min()
        dmax = sum_daily_el_consumption["sum_cons_date"].max()
        if dmin is not None and dmax is not None:
            years = range(dmin.year, dmax.year + 1)
            ee_holidays = holidays.country_holidays("EE", years=years)
            # sum_cons_date is a date (UTC-based). We classify holiday by the same date in EE calendar.
            sum_daily_el_consumption["is_holiday"] = sum_daily_el_consumption["sum_cons_date"].map(
                lambda d: d in ee_holidays)
        else:
            sum_daily_el_consumption["is_holiday"] = False
    else:
        sum_daily_el_consumption["is_holiday"] = pd.Series(dtype="boolean")
except Exception as _e:
    print("[warn] holidays library missing or failed; set daily is_holiday=False. Install: pip install holidays")
    sum_daily_el_consumption["is_holiday"] = False
